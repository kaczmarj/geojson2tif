...
This script was written to convert nuclear segmentations from HoverNet to pyramidal
TIF images.
...
if __name__ == "__main__":
    ...
    p.add_argument("--thresh", type=float, help="Keep nuclei with probs > thresh", default=0.)
    args = p.parse_args()
    ...
    print(f"[geojson2tif] Keeping nuclei with probability > {thresh}")
    transformed = [
        {
            "index": row["properties"]["instance_id"],
            "type": "polygon",
            "coordinates": row["geometry"]["coordinates"],
            "label": {
                "name": row["properties"]["type_str"],
                # Add 1 so values are 1,2,3,4,5,6. Background is 0.
                "value": row["properties"]["type_int"] + 1,
            },
        }
        for row in geojson_data["features"] if row["properties"]["type_prob"] > thresh
    ]
